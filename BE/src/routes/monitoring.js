...
router.get("/logs", asyncHandler(async (req, res) => {
  ...
  const result = await query(
    `SELECT l.log_id, l.log_style, l.message, l.created_at, l.batch_id, l.sensor_id, l.control_id, l.app_user_id, l.value,
     ...
     LEFT JOIN sensor_device sd ON sd.sensor_id = l.sensor_id
     LEFT JOIN control_device cd ON cd.control_id = l.control_id
     WHERE (${scopeClause} OR l.batch_id IS NULL)
       AND ($1::int IS NULL OR l.batch_id = $1)
       AND ($2::int IS NULL OR COALESCE(b.dry_id, sd.dry_id, cd.dry_id) = $2)
...
router.get("/dashboard/overview", asyncHandler(async (req, res) => {
  ...
  const batchStatQ = req.user.isAdmin
    ? `SELECT SUM(CASE WHEN status='running' THEN 1 ELSE 0 END)::int AS running_batches,
              ...
              SUM(CASE WHEN status IN ('failed', 'aborted') THEN 1 ELSE 0 END)::int AS failed_batches
  ...
  const result = await query(
    `SELECT l.created_at, l.value, sd.sensor_type, l.sensor_id, b.batch_id
     ...
     WHERE l.log_style = 'parameter_change' AND ${scopeClause}
       ...
       AND ($4::timestamptz IS NULL OR l.created_at <= $4)
       AND sd.sensor_type IN ('temperature', 'humidity', 'light')
...
router.get("/reports/quality", asyncHandler(async (req, res) => {
  ...
  const result = await query(
    `SELECT b.batch_id, b.dry_id,
            ...
            AVG(CASE WHEN sd.sensor_type='light' THEN l.value END) AS avg_light,
            SUM(CASE WHEN l.log_style='device_action' AND l.message LIKE 'Control %' THEN 1 ELSE 0 END)::int AS manual_actions
...
router.get("/reports/sensor-stats", asyncHandler(async (req, res) => {
  const dryerIds = req.user.isAdmin ? null : await getAccessibleDryerIds(req.user);
  if (!req.user.isAdmin && !dryerIds.length) return ok(res, []);
  const scopeClause = req.user.isAdmin ? "1=1" : `s.dry_id IN ${inClauseFromIds(dryerIds)}`;
  const { dry_id, from, to } = req.query;
  const result = await query(
    `SELECT sd.sensor_type,
            COUNT(l.log_id)::int              AS reading_count,
            COUNT(l.log_id)::int              AS count,
            ROUND(AVG(l.value)::numeric, 2)   AS avg_value,
            ROUND(AVG(l.value)::numeric, 2)   AS avg,
            ROUND(MIN(l.value)::numeric, 2)   AS min_value,
            ROUND(MIN(l.value)::numeric, 2)   AS min,
            ROUND(MAX(l.value)::numeric, 2)   AS max_value,
            ROUND(MAX(l.value)::numeric, 2)   AS max,
            ROUND(STDDEV(l.value)::numeric, 2) AS stddev_value,
            ROUND(STDDEV(l.value)::numeric, 2) AS stddev,
            ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY l.value)::numeric, 2) AS median_value,
            ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY l.value)::numeric, 2) AS median
     FROM log l
     JOIN sensor_device sd ON sd.sensor_id = l.sensor_id
     JOIN sensor_device s  ON s.sensor_id  = l.sensor_id
     WHERE l.log_style = 'parameter_change'
       AND ${scopeClause}
       AND ($1::int IS NULL OR s.dry_id = $1)
       AND ($2::timestamptz IS NULL OR l.created_at >= $2)
       AND ($3::timestamptz IS NULL OR l.created_at <= $3)
     GROUP BY sd.sensor_type
     ORDER BY sd.sensor_type`,
    [dry_id ? Number(dry_id) : null, from ? new Date(from) : null, to ? new Date(to) : null]
  );
  return ok(res, result.rows);
}));

router.get("/reports/fruit-usage", asyncHandler(async (req, res) => {
  const dryerIds = req.user.isAdmin ? null : await getAccessibleDryerIds(req.user);
  if (!req.user.isAdmin && !dryerIds.length) return ok(res, []);
  const scopeClause = req.user.isAdmin ? "1=1" : `b.dry_id IN ${inClauseFromIds(dryerIds)}`;
  ...
  const result = await query(
    `SELECT f.fruit_id, f.fruit_name,
            COUNT(b.batch_id)::int AS total_batches,
            SUM(CASE WHEN b.status='completed' THEN 1 ELSE 0 END)::int AS completed_batches,
            SUM(CASE WHEN b.status IN ('failed','aborted') THEN 1 ELSE 0 END)::int AS failed_batches,
            SUM(CASE WHEN b.status='running' THEN 1 ELSE 0 END)::int AS running_batches
     FROM fruit f
     LEFT JOIN batch b ON b.fruit_id = f.fruit_id
       AND (${scopeClause})
