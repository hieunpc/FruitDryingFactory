...
router.get("/recipes", asyncHandler(async (req, res) => {
  ...
  const result = await query(
    `SELECT r.recipe_id, r.recipe_name, r.recipe_type, r.fruit_id, r.is_active, r.created_at, f.fruit_name
     FROM recipe r
     LEFT JOIN fruit f ON r.fruit_id = f.fruit_id
     WHERE ($1::int IS NULL OR r.fruit_id = $1) AND ($2::boolean IS NULL OR r.is_active = $2)
...
router.delete("/recipes/:id", requireAdmin, asyncHandler(async (req, res) => {
  ...
  const recipeResult = await query(`SELECT recipe_id FROM recipe WHERE recipe_id = $1`, [recipeId]);
  if (!recipeResult.rows.length) throw new HttpError(404, "NOT_FOUND", "Recipe not found");
  
  const batchResult = await query(`SELECT 1 FROM batch WHERE recipe_id = $1 LIMIT 1`, [recipeId]);
  
  if (batchResult.rows.length > 0) {
    await query(`UPDATE recipe SET is_active = false WHERE recipe_id = $1`, [recipeId]);
    return ok(res, { message: "Recipe is used in batches, soft-deleted (marked inactive)", softDeleted: true });
  } else {
    const client = await getClient();
    try {
      await client.query("BEGIN");
      
      const phaseIdsResult = await client.query(`SELECT phase_id FROM phase WHERE recipe_id = $1`, [recipeId]);
      const phaseIds = phaseIdsResult.rows.map(r => r.phase_id);
      
      if (phaseIds.length > 0) {
        await client.query(`DELETE FROM policy_action WHERE policy_id IN (SELECT policy_id FROM policy WHERE phase_id = ANY($1::int[]))`, [phaseIds]);
        await client.query(`DELETE FROM policy_condition WHERE policy_id IN (SELECT policy_id FROM policy WHERE phase_id = ANY($1::int[]))`, [phaseIds]);
        await client.query(`DELETE FROM policy WHERE phase_id = ANY($1::int[])`, [phaseIds]);
        await client.query(`DELETE FROM phase_actions WHERE phase_id = ANY($1::int[])`, [phaseIds]);
      ...
      await client.query("COMMIT");
      return ok(res, { message: "Recipe deleted successfully", softDeleted: false });
    } catch (err) {
      await client.query("ROLLBACK");
      throw err;
    } finally {
