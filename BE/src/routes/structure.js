...
router.get("/dryers/:id", asyncHandler(async (req, res) => {
  ...
  const batches = await query(
    `SELECT status FROM batch 
     WHERE dry_id = $1 AND status IN ('running', 'paused', 'pending', 'scheduled')`,
    [dryId]
  );
  const hasRunningBatch = batches.rows.some(b => b.status === 'running' || b.status === 'paused');
  const hasPendingBatch = batches.rows.some(b => b.status === 'pending' || b.status === 'scheduled');
  const hasFanOn = controls.rows.some(c => c.control_type === 'fan' && (c.status === 'active' || c.status === 'on'));
  const hasLampOn = controls.rows.some(c => c.control_type === 'lamp' && (c.status === 'active' || c.status === 'on'));

  return ok(res, {
    ...dryer,
    sensors: sensors.rows,
    controls: controls.rows,
    has_running_batch: hasRunningBatch,
    has_pending_batch: hasPendingBatch,
    has_fan_on: hasFanOn,
    has_lamp_on: hasLampOn,
  });
}));
