...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (userId === req.user.id) {
      throw new HttpError(400, "BAD_REQUEST", "You cannot delete yourself");
    }

    try {
      // 1. Delete associated scopes first
      await query("DELETE FROM user_scope WHERE app_user_id = $1", [userId]);
      // 2. Delete report exports associated with the user
      await query("DELETE FROM report_export WHERE app_user_id = $1", [userId]);
      // 3. Try to delete the user
      await query("DELETE FROM app_user WHERE app_user_id = $1", [userId]);

      await writeLog({
        logStyle: "audit_config_change",
        message: `Admin ${req.user.email} deleted user ${userId}`,
        appUserId: req.user.id,
      });

      return ok(res, { message: "User deleted successfully" });
    } catch (err) {
      // Check for PostgreSQL foreign key violation code (23503)
      if (err.code === "23503") {
        throw new HttpError(
          400,
          "FOREIGN_KEY_VIOLATION",
          "Cannot delete this user because they have associated batch or log history. Please deactivate them instead."
        );
      }
      throw err;
    }
