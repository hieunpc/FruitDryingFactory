...
const { getBatchPhaseInfo, executePhaseActions, turnOffAllDevices } = require("../services/batchService");
...
router.get("/batches/:id", asyncHandler(async (req, res) => {
  ...
  const thresholdConditions = (await query(
    `SELECT pc.condition_id, pc.sensor_id, pc.value, pc.cp_operator, p.policy_id
     FROM policy_condition pc
     JOIN policy p ON p.policy_id = pc.policy_id
     JOIN phase ph ON ph.phase_id = p.phase_id
     WHERE ph.recipe_id = $1 AND p.policy_type = 'threshold_condition'`,
    [batch.recipe_id]
  )).rows;

  const policyIds = thresholdConditions.map(c => c.policy_id);
  let actions = [];
  if (policyIds.length > 0) {
    actions = (await query(
      `SELECT policy_id, control_id, action_type
       FROM policy_action
  ...
  const thresholdConditionsWithActions = thresholdConditions.map(cond => {
    return {
      sensor_id: cond.sensor_id,
      value: cond.value,
      cp_operator: cond.cp_operator,
      condition_id: cond.condition_id,
      actions: actions.filter(act => act.policy_id === cond.policy_id).map(act => ({
        control_id: act.control_id,
  ...
  const response = {
    ...
    threshold_conditions: thresholdConditionsWithActions,
  };
...
router.post("/batches/:id/start", asyncHandler(async (req, res) => {
  ...
  if (batch.operation_mode === "scheduled") {
    ...
    if (firstPhaseResult.rows.length) {
      ...
      await executePhaseActions(batchId, firstPhase.phase_id, batch.dry_id);
      await writeLog({
        logStyle: "batch_running",
        message: `Batch ${batchId} entered phase 1 (${firstPhase.duration_seconds}s)`,
...
router.post("/batches/:id/abort", asyncHandler(async (req, res) => {
  ...
  await turnOffAllDevices(batchId, batch.dry_id);
...
router.patch("/batches/:id/threshold-condition", asyncHandler(async (req, res) => {
  ...
  const policyResult = await query(
    `SELECT p.policy_id 
     FROM policy p
     JOIN policy_condition pc ON pc.policy_id = p.policy_id
     WHERE p.phase_id = $1 AND pc.sensor_id = $2
     LIMIT 1`,
    [phaseId, Number(sensor_id)]
  );
  ...
  if (policyResult.rows.length) {
    ...
    const newPolicy = await query(
      `INSERT INTO policy (policy_type, policy_name, phase_id, is_active) 
       VALUES ('threshold_condition', 'Threshold Policy for Batch ' || $1 || ' Sensor ' || $3, $2, TRUE)
       RETURNING policy_id`,
      [batchId, phaseId, Number(sensor_id)]
    );
