...
-- Update log_style check constraint to allow 'batch_pause', 'batch_resume', 'batch_abort'
ALTER TABLE log DROP CONSTRAINT IF EXISTS log_log_style_check;
ALTER TABLE log ADD CONSTRAINT log_log_style_check CHECK (log_style IN (
    'parameter_change', 'sensor_trigger', 'batch_start', 'batch_running', 'batch_pause', 'batch_resume', 'batch_abort', 'batch_end', 'device_action', 'audit_login', 'audit_permission_change', 'audit_config_change'
));